...
var checkinExchange = function(override) {
  var self = this;
  override = override || {};
  return getChannel.call(self).then(function(ch) {
    ...
    return ok.then(function(eok) {
      ...
      var ref = {exchangeName: _exchangeName, routingKey: _routingKey, channel: ch};
      if (self.config.exchangeMutex) {
        self.store.mutex = self.store.mutex || locks.createMutex();
        return new Promise(function(resolved, rejected) {
          self.store.mutex.lock(function() {
            resolved(ref);
          });
        });
      }
...
var checkoutExchange = function() {
  var self = this;
  if (self.config.exchangeMutex && self.store.mutex) {
...
var sendToQueue = function(data, opts) {
  ...
  return getChannel.call({ config: self.config, store: self.store }).then(function(ch) {
    return assertQueue.call({ config: self.config }, ch).then(function() {
      return ch.sendToQueue(self.config.queue, new Buffer(stringifyData(data)), opts);
...
Handler.prototype.produce = function(data, opts, override) {
  ...
  var producerCtx = { config: self.config, store: getProducerState.call(self) }
  return checkinExchange.call(producerCtx, override).then(function(ref) {
    return ref.channel.publish(ref.exchangeName, ref.routingKey, new Buffer(stringifyData(data)), opts);
  }).then(function(result) {
    checkoutExchange.call(producerCtx);
    return result;
  }).then(function(result) {
    if (!result) {
      return Promise.reject({
...
Handler.prototype.pull = function(stream, opts, override) {
  var self = this;
  opts = opts || {};
  debugx.enabled && debugx('extract() data from a readable stream');
  var producerCtx = { config: self.config, store: getProducerState.call(self) }
  return checkinExchange.call(producerCtx, override).then(function(ref) {
    var count = 0;
    var writable = true;
    var doRead = function() {
      var data = stringifyData(stream.read());
      writable = ref.channel.publish(ref.exchangeName, ref.routingKey, new Buffer(data), opts);
      debug2.enabled && debug2('send message#%s: %s', count++, writable);
    }
    return new Promise(function(resolved, rejected) {
      debug2.enabled && debug2('starting...');
      stream.on('readable', function() {
        if(writable) {
          doRead();
        } else {
          ref.channel.removeListener('drain', doRead);
          ref.channel.once('drain', doRead);
        }
      });
      stream.on('end', function() {
        ref.channel.removeListener('drain', doRead);
        debug2.enabled && debug2('stream end. Total: %s', count);
        resolved();
      });
      stream.on('error', function(err) {
        ref.channel.removeListener('drain', doRead);
        debug2.enabled && debug2('stream error: %s', JSON.stringify(err));
        rejected(err);
      });
    });
  }).then(function(result) {
