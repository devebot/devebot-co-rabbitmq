...
Handler.prototype.enqueue = function(data, opts, override) {
  opts = opts || {};
  override = override || {};
  ...
  debugx.enabled && debugx('enqueue() an object to rabbitmq');
  return getChannel.call({ config: self.config, store: self.publisherState }).then(function(ch) {
    var dataStr = null;
    if (typeof(data) === 'string') {
      dataStr = data;
      ...
      try {
        ...
        debugx.enabled && debugx('enqueue() invalid JSON object: %s', JSON.stringify(err));
      }
    }
    if (!dataStr) {
      return Promise.reject({
    ...
    var result = ch.publish(_exchangeName, _routingKey, new Buffer(dataStr), opts);
    if (!result) {
      return Promise.reject({
        failed: 1,
        successful: 0,
...
Handler.prototype.publish = Handler.prototype.enqueue;
...
Handler.prototype.process = function(callback) {
  ...
  debugx.enabled && debugx('process() an object from rabbitmq');
  return getChannel.call({ config: self.config, store: self.consumerState }).then(function(ch) {

    if (self.config.prefetch && self.config.prefetch >= 0) {
      debugx.enabled && debugx('process() - set channel prefetch: %s', self.config.prefetch);
    ...
    ok = ok.then(function(qok) {
      ...
      return ch_bindQueue(qok.queue, self.config.exchange, self.config.routingKey, {}).then(function() {
        debugx.enabled && debugx('process() - queue: %s has been bound', qok.queue);
        return qok;
      });
    });

    ok = ok.then(function(qok) {
      debugx.enabled && debugx('process() - queue info: %s', JSON.stringify(qok));
      if (self.config.maxConsumers && self.config.maxConsumers <= qok.consumerCount) {
        ...
        debugx.enabled && debugx('process() - Error: %s', JSON.stringify(error));
      ...
      return ch_consume(qok.queue, function(msg) {
        ...
        debugx.enabled && debugx('process() - received message: %s, fields: %s, amount: %s', 
        ...
        if (callback && typeof(callback) === 'function') {
          ...
          debugx.enabled && debugx('process() - callback (observer) is not available');
        }
      }, {noAck: self.config.noAck});
    });

    return ok.then(function(result) {
      debugx.enabled && debugx('process() - consumerTag: %s. CTRL+C to exit', result.consumerTag);
...
Handler.prototype.consume = Handler.prototype.process;
