...
var Handler = function(params) {
  ...
  Object.defineProperty(this, 'outlet', {
    get: function() { return self.producerState; },
    set: function(value) {}
  });
...
Handler.prototype.ready = Handler.prototype.prepare = function() {
  var self = this;
  self.producerState = self.producerState || new EventEmitter();
  return getChannel.call({ config: self.config, store: self.producerState }).then(function(ch) {
...
Handler.prototype.produce = function(data, opts, override) {
  ...
  return getChannel.call({ config: self.config, store: self.producerState }).then(function(ch) {
    var ok = assertExchange.call({ config: self.config, store: self.producerState }, ch);
    return ok.then(function(eok) {
      ...
      if (typeof(data) === 'string') {
        ...
        try {
          ...
          debugx.enabled && debugx('produce() invalid JSON object: %s', JSON.stringify(err));
        }
      }
      if (!dataStr) {
        return Promise.reject({
...
Handler.prototype.recycle = function(callback) {
  ...
  return getChannel.call({ config: recyclerCfg, store: self.recyclerState }).then(function(ch) {
    ...
    var ok = assertQueue.call({ config: recyclerCfg }, ch).then(function(qok) {
      ...
      if (recyclerCfg.maxConsumers && recyclerCfg.maxConsumers <= qok.consumerCount) {
        var error = {
          consumerCount: qok.consumerCount,
          maxConsumers: recyclerCfg.maxConsumers,
          message: 'exceeding quota limits of consumers'
        }
