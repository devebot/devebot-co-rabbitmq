...
var enqueueChain = function(data, opts) {
  ...
  var consumerCfg = self.config.consumer;
  self.consumerState = self.consumerState || {};
  return sendToQueue.call({ name: 'enqueueChain', config: consumerCfg, store: self.consumerState }, data, opts);
}
...
var enqueueTrash = function(data, opts) {
  ...
  var recyclerCfg = self.config.recycler;
  self.recyclerState = self.recyclerState || {};
  return sendToQueue.call({ name: 'enqueueTrash', config: recyclerCfg, store: self.recyclerState }, data, opts);
}
...
var assertRecycler = function() {
  ...
  self.recyclerState = self.recyclerState || {};
  return Promise.resolve({ config: recyclerCfg, store: self.recyclerState });
}
