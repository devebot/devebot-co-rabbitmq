...
SubscriberStream.prototype._next = function(content, callback) {
  var self = this;
  var data = stringifyData(content);
  self._ctx.writable = this._ref.channel.publish(this._ref.exchangeName, this._ref.routingKey, new Buffer(data), this._opts);
              ...
              if (hasRecycler.call(self)) {
                if (err) {
                  ...
                  if (headers[recyclerCfg.redeliveredCountName] <= recyclerCfg.redeliveredLimit) {
                    debugx.enabled && debugx('consume() - requeue message');
                    requeue.call(self, msg.content.toString(), {headers: headers});
                  } else {
                    debugx.enabled && debugx('consume() - discard message');
...
Handler.prototype.destroy = function() {
  ...
  return Promise.resolve().then(function() {
    ...
    return stopSubscriber.call({store: self.consumerState});
  }).then(function() {
    return stopSubscriber.call({store: self.recyclerState});
  });
}
