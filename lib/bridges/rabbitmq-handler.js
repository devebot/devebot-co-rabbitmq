...
var Handler = function(params) {
  ...
  config.queueName = params.queueName || params.queue || '';
  ...
  if (params.recycler && typeof(params.recycler) === 'object') {
    ...
    if (lodash.isEmpty(config.recycler.queueName)) {
      config.recycler.queueName = config.queueName + '-trashed';
    }
  }
...
var checkinExchange = function(override) {
  ...
  return getChannel.call(self).then(function(ch) {
    ...
    return ok.then(function(eok) {
      var _exchangeName = self.config.exchangeName;
...
var sendToQueue = function(data, opts) {
  ...
  return getChannel.call({ config: self.config, store: self.store }).then(function(ch) {
    return assertQueue.call({ config: self.config }, ch).then(function() {
      return ch.sendToQueue(self.config.queueName, new Buffer(stringifyData(data)), opts);
...
Handler.prototype.exhaust = function(stream, opts, override) {
  ...
  return checkinExchange.call(producerCtx, override).then(function(ref) {
    ...
    var doRead = function() {
      ...
      debug2.enabled && debug2('exhaust() - send message#%s: %s', count++, writable);
    }
    return new Promise(function(resolved, rejected) {
