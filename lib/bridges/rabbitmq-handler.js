...
var getConnect = function() {
  ...
  self.store.count = self.store.count || 0;
  debuglog.isEnabled && debuglog('getConnect() - connection amount: %s', self.store.count);
  ...
  if (self.store.connection) {
    ...
    return new Promise(function(onResolved, onRejected) {
      amqp.connect(self.config.host, function(err, conn) {
        ...
        debuglog.isEnabled && debuglog('getConnect() - connecting successfully');
