...
var debugx = debug('devebot:co:rabbitmq:handler');
var debug0 = debug('trace:devebot:co:rabbitmq:handler');
var debug2 = debug('devebot:co:rabbitmq:handler:extract');
...
var sendToQueue = function(data, opts) {
  ...
  var sendTo = function(ch) {
    self.config.sendable = ch.sendToQueue(self.config.queueName, zapper.bufferify(data), opts);
  }
